-- Raptor Stalk Behavior
--------------------------------------------------------------------------
local Behavior = CreateAIBehavior("RaptorStalk", -- "ParentBehavior",
{
  -- Optional - indicates the alert status (0 = idle, 1 = alert, 2 = engaged in combat)
	...
	Constructor = function (self, entity, data)
		...
		if (rand > 8) then
			entity:SelectPipe(0, "raptor_flank_right_1");
		elseif (rand > 5) then
			entity:SelectPipe(0, "raptor_flank_left_1");
		elseif (rand > 3) then
			entity:SelectPipe(0, "raptor_search_approach_1");
		else
			entity:SelectPipe(0, "raptor_search_approach_2");
		end
	...
	OnBulletRain = function(self, entity, sender)	
		...
		-- tell all nearby members to attack
		-- if no one else is around, run away..else attack with my friends
		
		AI.Signal(SIGNALFILTER_GROUPONLY, -1, "RAPTOR_ATTACK",entity.id); --sends signal to everyone else in the group to attack
		--if(AI_Raptor_Utils:GetCloseGroupMembers(entity) == 0) then
	...
	--------------------------------------------------
	OnSomebodyDied = function(self, entity, sender) -- 20% chance of running from player
	...
	OnTPSDestReached = function(self, entity) 
		...
		if (random(1,10) > 9) then --10% chance of attack after each move
			AI.Signal(SIGNALFILTER_GROUPONLY, -1, "RAPTOR_ATTACK",entity.id); --sends signal to everyone else in the group to attack
		end
		
	end,
	
	RAPTOR_ATTACK = function (self, entity, sender)
		--Log(entity:GetName() .. " Atttttack");
		AI.SetBehaviorVariable(entity.id, "AttackPlayer", true);

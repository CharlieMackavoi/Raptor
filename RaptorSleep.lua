-- Raptor Sleep Behavior
--------------------------------------------------------------------------
local Behavior = CreateAIBehavior("RaptorSleep", -- "ParentBehavior",
{
  -- Optional - indicates the alert status (0 = idle, 1 = alert, 2 = engaged in combat)
	...
	Constructor = function (self, entity, data)
		...
		if (random(1,10) > 8) then  --20% chance of sleeping raptor
			--entity:SelectPipe(0, "raptor_flank_right_1");
			entity:SelectPipe(0, "raptor_sleep");
		else
			AI.SetBehaviorVariable(entity.id, "Awake", true);
		end
	...
	OnInterestingSoundHeard = function(self, entity)
	    ...
	    --Log(entity:GetName() .. " - I heard you and waking up");		
		AI.SetBehaviorVariable(entity.id, "Awake", true);
		AI.SetBehaviorVariable(entity.id, "AwareOfPlayer", true);
	
	end,

	OnEnemyHeard = function(self, entity)
		-- called when the AI hears an interesting sound
		--Log(entity:GetName() .. " - I heard you and waking up");
